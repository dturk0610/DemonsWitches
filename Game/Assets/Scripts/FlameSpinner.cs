...
public class FlameSpinner : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ( flame1.localPosition.x - f1NextPos.x <= 0) flame1.GetComponent<Renderer>().sortingOrder = 5;
        else flame1.GetComponent<Renderer>().sortingOrder = 6;
        flame1.localPosition = f1NextPos;

        Vector3 f2NextPos = new Vector3( Mathf.Cos(currTime + xdiff)*.16f, .04f*Mathf.Sin(currTime + ydiff), 0 );
        if ( flame2.localPosition.x - f2NextPos.x <= 0) flame2.GetComponent<Renderer>().sortingOrder = 5;
        else flame2.GetComponent<Renderer>().sortingOrder = 6;
        ...
        if ( flame3.localPosition.x - f3NextPos.x <= 0) flame3.GetComponent<Renderer>().sortingOrder = 5;
        else flame3.GetComponent<Renderer>().sortingOrder = 6;
